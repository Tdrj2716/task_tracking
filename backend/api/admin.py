...
from .models import User, Project, Tag, Task, TimeEntry
...
class TimeEntryAdminForm(forms.ModelForm):
    """
    Custom form for TimeEntry admin to filter task and project by user
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # If time entry already has a user or user is being set
        if self.instance and self.instance.user_id:
            user = self.instance.user

            # Filter task choices to only show tasks from the same user
            self.fields["task"].queryset = Task.objects.filter(user=user)

            # Filter project choices to only show projects from the same user
            self.fields["project"].queryset = Project.objects.filter(user=user)
        elif "user" in self.data:
            # For new time entries, filter based on selected user
            try:
                user_id = int(self.data.get("user"))
                self.fields["task"].queryset = Task.objects.filter(user_id=user_id)
                self.fields["project"].queryset = Project.objects.filter(
                    user_id=user_id
                )
            except (ValueError, TypeError):
